...
public class ResourceCentre {
	...
	public static void main(String[] args) {
		...
		while (option != OPTION_QUIT) {
			...
			if (option == OPTION_VIEW) {
				...
				addItem(camcorderList, chromebookList, itemType);
				...
				itemTypeMenu();
				int itemType = Helper.readInt("Enter option to select item type > ");
				ResourceCentre.loanItem(camcorderList, chromebookList, itemType);
				...
				itemTypeMenu();
				int itemType = Helper.readInt("Enter option to select item type > ");
				ResourceCentre.returnItem(camcorderList, chromebookList, itemType);
	...
	private static void addItem(ArrayList<Camcorder> camcorderList, ArrayList<Chromebook> chromebookList,
			int itemType) {
		if (itemType == ITEM_TYPE_CHROMEBOOK) {
			Chromebook cb = inputChromebook();
			ResourceCentre.addChromebook(chromebookList, cb);
			System.out.println("Chromebook added");
		} else if (itemType == ITEM_TYPE_CAMCODER) {
	...
	private static void loanItem(ArrayList<Camcorder> camcorderList, ArrayList<Chromebook> chromebookList,
			int itemType) {
		if (itemType == ITEM_TYPE_CHROMEBOOK) {
			ResourceCentre.loanChromebook(chromebookList);
			System.out.println("Chromebook loaned");
		} else if (itemType == ITEM_TYPE_CAMCODER) {
			ResourceCentre.loanCamcorder(camcorderList);
			System.out.println("Camcorder loaned");
		} else {
			System.out.println("Invalid type");
		}
	}
	
	private static void returnItem(ArrayList<Camcorder> camcorderList, ArrayList<Chromebook> chromebookList,
			int itemType) {
		if (itemType == ITEM_TYPE_CHROMEBOOK) {
			...
			System.out.println("Camcorder returned");
		} else {
			System.out.println("Invalid type");
		}
	}
	...
	public static String retrieveAllCamcorder(ArrayList<Camcorder> camcorderList) {
		...
		for (int i = 0; i < camcorderList.size(); i++) {
			output += String.format("%-80s", camcorderList.get(i).toString());
		}
		return output;
	}
	...
	public static String retrieveAllChromebook(ArrayList<Chromebook> chromebookList) {
		...
		for (int i = 0; i < chromebookList.size(); i++) {
			output += String.format("%-80s", chromebookList.get(i).toString());
		}
		return output;
	}
